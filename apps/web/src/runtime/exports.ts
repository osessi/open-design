...
export function exportAsJsx(source: string, title: string): void {
  const blob = new Blob([source], { type: 'text/jsx;charset=utf-8' });
  triggerDownload(blob, `${safeFilename(title, 'component')}.jsx`);
}
...
export function exportReactComponentAsZip(source: string, title: string): void {
  const slug = safeFilename(title, 'component');
  const blob = buildZip([
    { path: `${slug}/${slug}.jsx`, content: source },
    {
      path: `${slug}/README.md`,
